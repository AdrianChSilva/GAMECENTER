...
class controladorVideojuego extends modeloVideojuego
{
    ...
    public function paginadorVideojuegoController($pagina, $registros, $codigo, $busq)
    {
        ...
        if (isset($busq) && $busq != "") {
            $consulta2 = "SELECT SQL_CALC_FOUND_ROWS * FROM videojuego
                       WHERE ((vidCodigo!='$codigo') AND (vidTitulo LIKE '%$busq%' 
                       OR vidPais LIKE '%$busq%' OR vidCodigo LIKE '%$busq%'))  
            ...
            foreach ($datos as $rows) {
                ...
                if ($_SESSION['tipoGC'] == "Administrador") {
                    $tabla .= '<a href="' . SERVERURL . 'videojuegoinfo/' . $rows['vidCodigo'] . '" class="btn btn-primary" title="Más información"><i class="zmdi zmdi-info"></i></a>
                        <a href="#!" class="btn btn-primary" title="Ver PDF. NO DISPONIBLE EN ESTA VERSIÓN"><i class="zmdi zmdi-file"></i></a>
                        <a href="#!" class="btn btn-primary" title="Descargar PDF. NO DISPONIBLE EN ESTA VERSIÓN"><i class="zmdi zmdi-cloud-download"></i></a>
